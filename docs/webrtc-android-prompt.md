# Prompt: Implémentation WebRTC Android
...
- [ ] Demande de permissions au runtime (permissions déjà accordées mais jamais demandées explicitement)
...
- [ ] `CallViewModel.kt` - State centralisé (state, remoteUser, isMuted, isCameraEnabled)
- [ ] Afficher vidéo locale (petit encart)
- [ ] Bouton mute micro
- [ ] Bouton toggle caméra on/off
- [ ] `IncomingCallActivity.kt` + `IncomingCallScreen.kt` - UI dédiée appel entrant
